 const SHEET_URL = "https://script.google.com/macros/s/AKfycbwUmk__yXC79NlyFGpSlx2NrifhsfUDxTSaHStA1-bLknItyimbHLLHl8fb3ArA23JZTQ/exec";
  ...
  function getStoredImages() {
    try { return JSON.parse(localStorage.getItem('dnd_images') || '{}'); } catch { return {}; }
  }
  ...
  async function loadData() {
    try {
      ...
      if (grid) {
        grid.innerHTML = `<div class="empty-msg" style="color:#ef5350;">Aviso: No se pudieron sincronizar los datos online (${e.message}). <br><small>Comprobá que el script de Google sea público.</small></div>`;
      }
    }
  }
  ...
  function hpColor(p) { return p > 0.6 ? '#4caf50' : p > 0.3 ? '#ff9800' : '#ef5350'; }
  ...
  function calcCombat() {
    ...
    if (!val) { out.textContent='Selecciona un atacante'; out.style.color='#666'; return; }
    const [atk,,name] = val.split('|');
    const minRoll = ac - parseInt(atk);
    let prob, msg;
    if (minRoll<=1)      { prob=100; msg='¡Impacto automático!'; }
    else if (minRoll>20) { prob=5;   msg='Solo nat 20 (5%)'; }
    else { prob=Math.round((21-minRoll)/20*100); msg=`${prob}% de probabilidad de impacto`; }
  ...
  function rollAttack() {
    ...
    const ac  = parseInt(document.getElementById('ac').value)||10;
    if (!val) { alert('Seleccioná un atacante primero.'); return; }
    const [atk, dmg, name] = val.split('|');
    const d20   = Math.ceil(Math.random()*20);
    const total = d20+parseInt(atk);
    const crit=d20===20, pifia=d20===1;
    ...
    if (crit)     entry.innerHTML=`<span class="crit">🎯 ¡CRÍTICO! ${name} — d20: ${d20} (+${atk}) = ${total} vs CA ${ac} — ${dmgRoll} daño (dado doble)</span>`;
    else if (hit) entry.innerHTML=`<span class="hit">✅ ${name} — d20: ${d20} (+${atk}) = ${total} vs CA ${ac} — ${dmgRoll} daño</span>`;
    else          entry.innerHTML=`<span class="miss">❌ ${name} — d20: ${d20} (+${atk}) = ${total} vs CA ${ac} — Falla${pifia?' (pifia)':''}</span>`;
  ...
  let selDie=20, rolling=false;

  function buildDiceGrid() {
    const g=document.getElementById('diceGrid'); if(!g) return;
    g.innerHTML='';
    DICE.forEach(d=>{
      const b=document.createElement('button');
      b.className='die-btn'+(d===selDie?' active':'');
      b.onclick=()=>{ if(!rolling){selDie=d;buildDiceGrid();buildArena();} };
  ...
  function polyShape(d,label) {
    const L=label!=null?label:'', fs=String(L).length>2?14:String(L).length>1?18:22;
    const f='#c3073f',s='#8a0029';
    if(d===4)   return `<svg viewBox="0 0 100 100"><polygon points="50,5 95,90 5,90" fill="${f}" stroke="${s}" stroke-width="3"/><text x="50" y="78" text-anchor="middle" font-size="${fs}" font-weight="700" fill="#fff">${L}</text></svg>`;
    if(d===8)   return `<svg viewBox="0 0 100 100"><polygon points="50,5 95,50 50,95 5,50" fill="${f}" stroke="${s}" stroke-width="3"/><text x="50" y="57" text-anchor="middle" font-size="${fs}" font-weight="700" fill="#fff">${L}</text></svg>`;
    if(d===10)  return `<svg viewBox="0 0 100 100"><polygon points="50,5 90,35 78,80 22,80 10,35" fill="${f}" stroke="${s}" stroke-width="3"/><text x="50" y="60" text-anchor="middle" font-size="${fs}" font-weight="700" fill="#fff">${L}</text></svg>`;
    if(d===12)  return `<svg viewBox="0 0 100 100"><polygon points="50,5 82,18 95,50 82,82 50,95 18,82 5,50 18,18" fill="${f}" stroke="${s}" stroke-width="3"/><text x="50" y="57" text-anchor="middle" font-size="${fs}" font-weight="700" fill="#fff">${L}</text></svg>`;
    if(d===20)  return `<svg viewBox="0 0 100 100"><polygon points="50,3 97,27 97,73 50,97 3,73 3,27" fill="${f}" stroke="${s}" stroke-width="3"/><text x="50" y="57" text-anchor="middle" font-size="${fs}" font-weight="700" fill="#fff">${L}</text></svg>`;
    if(d===100) return `<svg viewBox="0 0 100 100"><circle cx="50" cy="50" r="44" fill="${f}" stroke="${s}" stroke-width="3"/><text x="50" y="57" text-anchor="middle" font-size="${fs}" font-weight="700" fill="#fff">${L}</text></svg>`;
    return `<svg viewBox="0 0 100 100"><polygon points="50,5 95,90 5,90" fill="${f}" stroke="${s}" stroke-width="3"/><text x="50" y="78" text-anchor="middle" font-size="${fs}" font-weight="700" fill="#fff">${L}</text></svg>`;
  }

  function getFaceRot(v){const m={1:{rx:0,ry:0},2:{rx:0,ry:90},3:{rx:-90,ry:0},4:{rx:90,ry:0},5:{rx:0,ry:-90},6:{rx:180,ry:0}};return m[v]||{rx:0,ry:0};}
  ...
  function buildArena(result) {
    ...
    if(selDie===6){
      ...
      arena.innerHTML=`<div class="d6-wrap" id="d6" style="--rx:${f.rx}deg;--ry:${f.ry}deg;transform:rotateX(${f.rx}deg) rotateY(${f.ry}deg);"><div class="d6-face f1">1</div><div class="d6-face f2">6</div><div class="d6-face f3">2</div><div class="d6-face f4">5</div><div class="d6-face f5">3</div><div class="d6-face f6">4</div></div>`;
    } else {
  ...
  function rollDice() {
    ...
    if(selDie===6){
      ...
      if(el){const f=getFaceRot(first);el.style.setProperty('--rx',f.rx+'deg');el.style.setProperty('--ry',f.ry+'deg');el.classList.add('spinning');}
    } else {
      const el=document.getElementById('polyDie');
      if(el) el.classList.add('poly-spinning');
    }
    setTimeout(()=>{ buildArena(first); showDiceResult(rolls,mod); rolling=false; },1050);
  }

  function showDiceResult(rolls,mod) {
    const qty=rolls.length, sum=rolls.reduce((a,b)=>a+b,0), total=sum+mod;
    const pills=rolls.map(r=>`<span class="roll-pill ${r===selDie?'crit':r===1?'fail':''}">${r}</span>`).join('');
    const modStr=mod!==0?` ${mod>0?'+':''}${mod}`:'';
    const isCrit=qty===1&&rolls[0]===selDie, isFail=qty===1&&rolls[0]===1;
    ...
    entry.innerHTML=`<strong style="color:#e0e0e0;">${total}</strong> — ${qty}d${selDie}${modStr} [${rolls.join(', ')}]${isCrit?' 🔥 nat max':isFail?' 💀 pifia':''}`;
